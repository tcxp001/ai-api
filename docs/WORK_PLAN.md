...
后续推进必须遵守：**不是功能越多越好，每个功能都必须让核心使用路径更短、更清楚、更少干扰。**

页面职责边界：

- Provider 维护页只做维护：新增、编辑、复制、删除、启用/停用、字段维护。
- Provider 测活页只做测活：选择 Provider/模型、测试、结果、错误诊断、修复建议。
- AIProxy 页只做服务管理：状态、启动、停止、重启、端口、服务配置。
- Codex/Hermes 配置能力单独形成闭环，不随意塞进 Provider 行操作。

禁止为了“顺手”增加跨职责按钮。例如：

- 不在 Provider 维护页增加测活按钮。
- 不在 Provider 维护页增加手动复制本地代理 URL 按钮。
- 不把低频调试动作放到主列表高频操作区。
